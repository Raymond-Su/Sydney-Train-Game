const evaluateRPNExpression = require('./evaluate');
const { inOrderCombinations, RPNToInfix } = require('./expressions');
const product = require('./product');

const basicOperators = ['/', '*', '-', '+'];
const advancedOperators = ['^'];
const allOperators = [...advancedOperators, ...basicOperators];

const findSolutions = (puzzle, goal = 10, operators = allOperators) => {
  ...
  const RPNSolutions = RPNExpressions.filter(
...
const isSolveable = (puzzle, target = 10) => {
  const n = Array.from(puzzle).length;

  const subProblems = Array(n);
  for (let i = 0; i < n; i += 1) {
